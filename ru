...
Logs|| Журналы
License|| Лицензия
Support Save|| Поддержка Сохранить
Create|| создать
Edit|| Редактировать
Configure|| Установка
Upload|| Загрузить
...
NIC Interfaces|| Интерфейсы NIC
Megabytes (MB)|| Мегабайт (МБ)
Farm List|| Список ферм
Name|| Имя
Profile|| Профиль
Status|| Статус
...
Message Error 414|| Сообщение об ошибке 414
Message Error 500|| Сообщение об ошибке 500
Message Error 501|| Сообщение об ошибке 501
Message Error 503|| Сообщение об ошибке 503
HTTPS Parameters|| Параметры HTTPS
Disable SSLv2|| Отключить SSLv2
Disable SSLv3|| Отключить SSLv3
Disable TLSv1|| Отключить TLSv1
Disable TLSv1.1|| Отключить TLSv1.1
Disable TLSv1.2|| Отключить TLSv1.2
Ciphers|| шифры
Custom Ciphers|| Пользовательские шифры
Certificate Enabled|| Сертификат включен
...
Load balancer Algorithm|| Алгоритм балансировки нагрузки
...
Farms Graphs|| Фермы Графики
Select Graphs|| Выберите Графики
...
Default is 8.8.8.8|| По умолчанию это 8.8.8.8
Optional|| Опция
Default is pool.ntp.org|| По умолчанию это pool.ntp.org
Primary server|| Основной сервер
Secondary server|| Вторичный сервер
NPT Service|| NPT Service
NPT Server|| Сервер NPT
SNMP Service|| Служба SNMP
Enable SNMP|| Включить SNMP
Physical interface where is running SNMP service|| Физический интерфейс, где работает служба SNMP
SNMP Port where is running SNMP service|| Порт SNMP, где работает служба SNMP
Default is 161.|| По умолчанию это 161.
Community name|| Имя сообщества
IP or subnet with access (IP/bit)|| IP или подсеть с доступом (IP / бит)
...
Netmask|| Netmask
MAC|| MAC
IP Address (IPv4 / IPv6)|| IP-адрес (IPv4 / IPv6)
VLAN tag|| Тег VLAN
Date|| Время
Log|| Журнал
...
found|| найденный
 ...
 not is valid, it is posible to define a single port, several ports or ranges of ports (Ex: 80 or 80,81 or 80:90)|| недопустимо, возможно определить один порт, несколько портов или диапазоны портов (например: 80 или 80,81 или 80: 90)
 has to be greater than 0|| должен быть больше чем 0
 can only has two characters.|| может иметь только два символа.
 has to be a valid email.|| должен быть действительный адрес электронной почты.
...
The farmguardian has been disabled sucessfully.|| Фермер был успешно отключен.
The farmguardian has been changed to {{param}} successfully.|| Хозяин фермы был успешно изменен на {{param}}.
The <strong>{{param}}</strong> farmguardian has been created successfully.||   <strong>{{param}}</strong> Farmguardian был создан успешно.
The <strong>{{param}}</strong> farmguardian has been updated successfully||   <strong>{{param}}</strong> Farmguardian был успешно обновлен
Are you sure you want to delete the {{param}} farmguardian?|| Вы уверены, что хотите удалить фермера {{param}}?
The <strong>{{param}}</strong> farmguardian has been deleted successfully.||   <strong>{{param}}</strong> Farmguardian был успешно удален.
The farm has been updated successfully.|| Ферма была успешно обновлена.
Are you sure you want to delete the farm {{param}} {{param2}} of the farmguardian?|| Вы уверены, что хотите удалить ферму {{param}} {{param2}} фермера?
...
The backend <strong>{{param}}</strong> has been {{param2}} successfully.|| Бэкэнд <strong>{{param}}</strong> успешно прошел {{param2}}.
...
The <strong>{{param}}</strong> Gateway has been updated successfully.||   <strong>{{param}}</strong> Шлюз был успешно обновлен.
Are you sure you want to unset the current gateway?|| Вы уверены, что хотите сбросить текущий шлюз?
Are you sure you want to unset the {{param}} gateway?|| Вы уверены, что хотите удалить шлюз {{param}}?
The <strong>{{param}}</strong> Gateway has been unconfigured successfully.||   <strong>{{param}}</strong> Шлюз был успешно настроен.
The <strong>{{param}}</strong> NIC has been updated successfully.||   <strong>{{param}}</strong> NIC был успешно обновлен.
The <strong>{{param}}</strong> NIC has been unconfigured successfully.||   <strong>{{param}}</strong> Сетевая карта успешно настроена.
The <strong>{{param}}</strong> NIC is {{param2}}||   <strong>{{param}}</strong> NIC - это {{param2}}
The <strong>{{param}}</strong> Virtual interface has been created successfully.||   <strong>{{param}}</strong> Виртуальный интерфейс был успешно создан.
...
The <strong>{{param}}</strong> VLAN has been deleted successfully.||   <strong>{{param}}</strong> VLAN был успешно удален.
The <strong>{{param}}</strong> VLAN is {{param2}}||   <strong>{{param}}</strong> VLAN - это {{param2}}
...
The backup <strong>{{backup}}</strong> has been downloaded successfully.|| Бэкап <strong>{{backup}}</strong> был успешно загружен.
The log <strong>{{param}}</strong> has been downloaded successfully.|| Журнал <strong>{{param}}</strong> был успешно загружен.
The <strong>{{param}}</strong> service has been updated successfully.||   <strong>{{param}}</strong> Сервис был успешно обновлен.
The <strong> Support Save </strong> has been downloaded successfully||   <strong> Поддержка Сохранить </strong> был успешно загружен
...
Generate Support Save|| Создать поддержку Сохранить
clear|| Очистить
start|| Начало
Apply|| применять
Add|| Добавить
...
No farms found|| Фермы не найдены
Established Conns|| Штатные Коннс
Pending Conns|| В ожидании Conns
Select a graph|| Выберите график
Select a profile|| Выберите профиль
Select a Virtual IP|| Выберите виртуальный IP
Select a parent interface|| Выберите родительский интерфейс
Select an interface|| Выберите интерфейс
Logout|| Выйти
